...
\begin{itemize}
	...
	\item With \emph{Other settings} the part with the file information can be hidden, as it's not used by all users. If this part is hidden, a single-line or single-column view is also possible. If all four areas are visible, this does not make sense. 
\end{itemize}
...
For more detailed editing of the meta information, Nemp has a \textbf{detail window} that can be opened with the key combination \texttt{Ctrl+D} when a title is highlighted in the media library or playlist. In addition to specific changes to the ID3 tags, other information can be retrieved here that is not necessarily of great interest, but some may find it useful.

\begin{hinweis} If you make changes in this window and click \emph{Apply}, the inputs are always saved in the metadata of the file, even if quick access to the metadata is not allowed in the settings.
\end{hinweis}
...
In the first tab \textbf{General} you get an overview of the metadata for the selected title. The button \emph{Explorer} opens the Windows Explorer with the directory of the file, via \emph{Properties} you receive the Windows properties dialog.
...
In the third tab you can edit \textbf{lyrics and pictures}. If you want to edit lyrics for several titles, it might be faster to use the multiple search for lyrics. To do this, select several files in the main window and select the \emph{Get  Lyrics} function (key combination \texttt{Ctrl+L}). This will take a little time, and not all titles will be found with the correct lyrics, but the hit rate is quite good.
...
Adding extended tags is the same as adding lyrics. You can enter a few tags manually, or search for tags for many files simultaneously. To do this, select the desired files and use the \emph{Get extended tags} function (key combination \texttt{Ctrl+T}). See also the section about the \hyperref[sec_tagwolkeneditor]{tag cloud editor}.
