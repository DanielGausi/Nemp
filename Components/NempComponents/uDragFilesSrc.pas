...
Slightly modified by Daniel Gaussmann
mail@gausi.de, April 2009
- Char-stuff for Delphi 2009
...
begin
  ...
  for i := 1 to FileList.count do
    inc(FileListBytes, length(FileList[i-1])*SizeOf(Char) + SizeOf(Char)); // String#0
end;
{=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=}
  ...
  if not Failed(QueryGetData(formatetcIn)) then begin
      ...
      for i := 1 to FileList.count do begin
        if i = FileList.count then
          //WstrPcopy( pWidechar(longint(ptrdropfile)+offset),  FileList[i-1]+#0#0)
          StrPcopy( PChar(longint(ptrdropfile) + offset),  FileList[i-1]+#0#0)
        else
          //WstrPcopy( pWidechar(longint(ptrdropfile)+offset),  FileList[i-1]+#0);
          StrPcopy( PChar(longint(ptrdropfile)+offset),  FileList[i-1]+#0);

        offset := offset + length(FileList[i-1])*SizeOf(Char) + SizeOf(Char);
      end;
...
procedure Register;
begin
  RegisterComponents('Beispiele', [TDragFilesSrc]);
end;
